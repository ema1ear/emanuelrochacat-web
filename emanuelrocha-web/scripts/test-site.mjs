...
assert.match(template, /id="intro-counter"[^>]*data-action="skip-intro"[^>]*hidden/);
assert.match(template, /id="intro-counter-value"[^>]*>000<\/span>/);
...
assert.match(appScript, /const INTRO_NAME_MS = 1800;/, "El nombre debe permanecer 1,8 segundos.");
assert.match(appScript, /const INTRO_WELCOME_MS = 900;/, "La bienvenida debe permanecer 0,9 segundos.");
assert.match(appScript, /const INTRO_HORIZONTAL_MS = 1400;/, "El desplazamiento horizontal debe disponer de 1,4 segundos.");
assert.match(appScript, /const INTRO_VERTICAL_MS = 1400;/, "El asentamiento vertical debe disponer de 1,4 segundos.");
assert.match(appScript, /setState\("welcome-settle"\)/, "La entrada debe separar los dos desplazamientos.");
assert.match(appScript, /if \(action === "skip-intro"\) finishIntro\(\{ focus: true \}\);/);
assert.match(appScript, /requestAnimationFrame\(update\)/, "El contador debe seguir la animación del navegador.");
...
assert.match(appScript, /mode: "operations-index"/);
assert.match(appScript, /mode: "contact"/);
assert.match(appScript, /mode: "complete"/);
assert.match(appScript, /function scheduleFocusAlignment\(/);
...
assert.match(introStyles, /\.intro-counter\s*\{[\s\S]*?position: absolute;/, "El contador no debe añadir altura al layout.");
assert.match(introStyles, /\.intro-counter\s*\{[\s\S]*?min-height: 44px;/, "El objetivo táctil móvil debe medir al menos 44 px.");
assert.match(introStyles, /@media \(min-width: 768px\)[\s\S]*?\.intro-counter \{ right: -56px; \}/, "En escritorio debe aprovechar el margen exterior.");
...
assert.match(introStyles, /\.focus-progress-dot\s*\{[\s\S]*?top: calc\(var\(--focus-progress\) \* 100%\);/, "El progreso debe moverse sin descentrar el núcleo focal.");
