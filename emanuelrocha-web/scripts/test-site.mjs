...
for (const page of routePages) {
    ...
    if (page.key === "home") {
        assert.equal((html.match(/<a[^>]+data-action="narrative"/g) ?? []).length, 2);
        assert.equal((html.match(/class="scrolling-p"/g) ?? []).length, 0);
        assert.equal((html.match(/class="operation-file-row"/g) ?? []).length, 0);
        assert.match(html, /id="form-intent-lead"><\/legend>/);
    }
    ...
    if (page.key === "operations") {
        ...
        assert.equal((html.match(/class="operation-file-row"/g) ?? []).length, 6);
        assert.equal((html.match(/class="scrolling-p"/g) ?? []).length, 0);
        assert.match(html, /id="prose-text"><\/p>/);
        assert.match(html, /id="form-intent-lead"><\/legend>/);
    }

    if (page.key === "A" || page.key === "B") {
        const paragraphs = (html.match(/class="scrolling-p"/g) ?? []).length;
        assert.equal(paragraphs, page.key === "A" ? 20 : 35, `Número de unidades incorrecto: ${page.lang}/${page.key}.`);
        assert.equal(
            (html.match(new RegExp(`<a[^>]+data-action="contact-from-narrative" data-intent="${page.key}"`, "g")) ?? []).length,
            page.key === "A" ? 2 : 1,
    ...
    if (page.key === "contact") {
        assert.equal((html.match(/class="scrolling-p"/g) ?? []).length, 0);
        assert.equal((html.match(/class="operation-file-row"/g) ?? []).length, 0);
        assert.doesNotMatch(html, /id="form-intent-lead"><\/legend>/);
        assert.match(html, /id="prose-text"><\/p>/);
    }
}
...
assert.match(template, /<a[^>]+id="brand-home"[^>]+href="\{\{HOME_URL\}\}"[^>]+data-action="home"/);
assert.match(template, /<a[^>]+id="btn-contact"[^>]+href="\{\{CONTACT_URL\}\}\?intent=A"[^>]+data-action="contact"/);
assert.match(template, /<a[^>]+id="btn-operations-footer"[^>]+href="\{\{OPERATIONS_URL\}\}"[^>]+data-action="operations"/);
assert.ok(
