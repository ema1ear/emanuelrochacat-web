...
const INTRO_NAME_MS = 1800;
const INTRO_WELCOME_MS = 900;
const INTRO_HORIZONTAL_MS = 1400;
const INTRO_VERTICAL_MS = 1400;
const INTRO_TOTAL_MS = INTRO_NAME_MS + INTRO_WELCOME_MS + INTRO_HORIZONTAL_MS + INTRO_VERTICAL_MS;
const elements = {
    ...
    introCounter: document.getElementById("intro-counter"),
    introCounterValue: document.getElementById("intro-counter-value"),
    focusCompanion: document.getElementById("focus-companion"),
    focusTicks: document.getElementById("focus-ticks"),
    welcomeFade: document.querySelector(".welcome-fade-block"),
...
let introTimers = [];
let introCounterFrame = null;
let introCounterHideTimer = null;
let introRunning = false;
let focusPulseTimer = null;
let focusAlignFrame = null;
let focusAlignTimer = null;
let focusSignature = "";
...
function renderLanguage() {
    ...
    elements.introCounter.setAttribute("aria-label", copy.skipIntro);
...
function operationIndexLinks() {
    return [...elements.cabinet.querySelectorAll("[data-operation-id]")];
}

function closestOperationIndex() {
    const links = operationIndexLinks();
    if (!links.length) return 0;
    const maximumScroll = Math.max(0, elements.operationsView.scrollHeight - elements.operationsView.clientHeight);
    if (elements.operationsView.scrollTop <= 1) return 0;
    if (maximumScroll - elements.operationsView.scrollTop <= 1) return links.length - 1;
    const view = elements.operationsView.getBoundingClientRect();
    const center = view.top + view.height / 2;
    return links.reduce((closest, link, index) => {
...
function setOperationIndexFocus(index) {
    const links = operationIndexLinks();
    const files = operationsFiles();
    const activeIndex = Math.min(Math.max(0, index), Math.max(0, files.length - 1));
    links.forEach((link, linkIndex) => link.classList.toggle("is-focal", linkIndex === activeIndex));

    setFocusCompanion({
        mode: "operations-index",
        progress: files.length > 1 ? activeIndex / (files.length - 1) : 1,
        steps: files.length,
        activeIndex
    });
}
...
function updateOperationIndexFocus() {
    ...
    const focusedLink = document.activeElement?.closest?.("[data-operation-id]");
    if (focusedLink && elements.cabinet.contains(focusedLink)) {
        setOperationIndexFocus(operationIndexLinks().indexOf(focusedLink));
        return;
    }
    setOperationIndexFocus(closestOperationIndex());
}
...
function activeSectionForState(state) {
    if (["intro", "welcome-center", "welcome-left", "welcome-settle", "welcome-ready"].includes(state)) return "view-welcome";
...
function syncAccessibility(state) {
    ...
    elements.footer.inert = ["intro", "welcome-center", "welcome-left", "welcome-settle"].includes(state);
...
function focusTargetForState() {
    const state = body.dataset.state;
    if (state === "welcome-ready") return elements.welcomeFade;
    if (state === "narrative") return elements.teleprompter;
    if (state === "contact") return activeFormStep() ?? elements.contactView;
    if (state === "operations") return elements.operationsView;
    return elements.main;
}

function visibleTargetCenter(target) {
    const targetRect = target.getBoundingClientRect();
    let top = targetRect.top;
    let bottom = targetRect.bottom;

    if (body.dataset.state === "contact") {
        const boundary = elements.contactView.getBoundingClientRect();
        top = Math.max(top, boundary.top);
        bottom = Math.min(bottom, boundary.bottom);
    }

    return bottom > top ? top + (bottom - top) / 2 : targetRect.top + targetRect.height / 2;
}

function alignFocusCompanion() {
    focusAlignFrame = null;
    const target = focusTargetForState();
    if (!target || elements.focusCompanion.dataset.mode === "hidden") return;

    const appRect = elements.app.getBoundingClientRect();
    const mainRect = elements.main.getBoundingClientRect();
    const halfHeight = elements.focusCompanion.offsetHeight / 2;
    const minimum = mainRect.top + halfHeight;
    const maximum = mainRect.bottom - halfHeight;
    const center = Math.min(maximum, Math.max(minimum, visibleTargetCenter(target)));
    elements.focusCompanion.style.setProperty("--focus-center-y", `${(center - appRect.top).toFixed(2)}px`);
}

function scheduleFocusAlignment() {
    if (focusAlignFrame !== null) cancelAnimationFrame(focusAlignFrame);
    if (focusAlignTimer !== null) clearTimeout(focusAlignTimer);
    focusAlignFrame = requestAnimationFrame(alignFocusCompanion);
    const state = body.dataset.state;
    const settleDelay = state === "welcome-ready" ? 780 : 1320;
    focusAlignTimer = setTimeout(() => {
        focusAlignTimer = null;
        if (focusAlignFrame !== null) cancelAnimationFrame(focusAlignFrame);
        focusAlignFrame = requestAnimationFrame(alignFocusCompanion);
    }, settleDelay);
}
...
function setFocusCompanion({ mode, progress = 0.5, steps = 0, activeIndex = null }) {
...
function syncFocusState(state) {
    if (["intro", "welcome-center", "welcome-left", "welcome-settle"].includes(state)) {
...
function scheduleIntro() {
    ...
    introTimers.push(setTimeout(
        () => setState("welcome-settle"),
        INTRO_NAME_MS + INTRO_WELCOME_MS + INTRO_HORIZONTAL_MS
    ));
    introTimers.push(setTimeout(finishIntro, INTRO_TOTAL_MS));
}

function setIntroCounter(value) {
    const normalized = Math.min(100, Math.max(0, Math.round(value)));
    elements.introCounterValue.textContent = String(normalized).padStart(3, "0");
    elements.introCounter.style.setProperty("--counter-progress", `${normalized / 100}`);
}

function stopIntroCounter() {
    if (introCounterFrame !== null) cancelAnimationFrame(introCounterFrame);
    introCounterFrame = null;
}
...
function cancelIntro() {
    ...
    introTimers = [];
    stopIntroCounter();
    if (introCounterHideTimer !== null) clearTimeout(introCounterHideTimer);
    introCounterHideTimer = null;
    introRunning = false;
    elements.introCounter.hidden = true;
    elements.introCounter.disabled = true;
    elements.introCounter.tabIndex = -1;
    elements.introCounter.classList.remove("is-complete");
}

function startIntroCounter() {
    const startedAt = performance.now();
    elements.introCounter.hidden = false;
    elements.introCounter.disabled = false;
    elements.introCounter.tabIndex = 0;
    elements.introCounter.classList.remove("is-complete");
    setIntroCounter(0);

    const update = (now) => {
        if (!introRunning) return;
        setIntroCounter(((now - startedAt) / INTRO_TOTAL_MS) * 100);
        introCounterFrame = requestAnimationFrame(update);
    };
    introCounterFrame = requestAnimationFrame(update);
}
...
function finishIntro({ focus = false } = {}) {
    ...
    elements.introCounter.disabled = true;
    elements.introCounter.tabIndex = -1;
    elements.introCounter.classList.add("is-complete");
    introCounterHideTimer = setTimeout(() => {
        if (!introRunning) elements.introCounter.hidden = true;
        introCounterHideTimer = null;
    }, 320);
    if (focus) requestAnimationFrame(() => elements.main.focus({ preventScroll: true }));
}
...
document.addEventListener("click", (event) => {
    ...
    if (action === "skip-intro") finishIntro({ focus: true });
...
function previewOperationLink(target) {
    const link = target.closest?.("[data-operation-id]");
    if (!link || body.dataset.state !== "operations" || currentOperationId()) return;
    const index = operationIndexLinks().indexOf(link);
    if (index >= 0) setOperationIndexFocus(index);
}

document.addEventListener("pointerover", (event) => previewOperationLink(event.target));
document.addEventListener("focusin", (event) => previewOperationLink(event.target));
document.addEventListener("pointerout", (event) => {
    if (event.target.closest?.("[data-operation-id]") && !event.relatedTarget?.closest?.("[data-operation-id]")) {
        scheduleOperationIndexFocus();
    }
});
document.addEventListener("focusout", (event) => {
    if (event.target.closest?.("[data-operation-id]")) scheduleOperationIndexFocus();
});
...
window.addEventListener("resize", scheduleFocusAlignment, { passive: true });
