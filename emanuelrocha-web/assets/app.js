...
function renderOperationsFooter(activeId) {
    ...
    if (body.dataset.state === "operations") {
        setFocusCompanion({
            mode: "operations",
            progress: reading && files.length > 1 ? index / (files.length - 1) : 0.5,
            steps: files.length,
            activeIndex: reading ? index : null
        });
    }
...
function setFocusCompanion({ mode, progress = 0.5, steps = 0, activeIndex = null }) {
    ...
    if (mode !== "hidden") pulseFocusCompanion();
}
...
function syncOperationsFocus() {
    ...
    setFocusCompanion({
        mode: "operations",
        progress: index >= 0 && files.length > 1 ? index / (files.length - 1) : 0.5,
        steps: files.length,
