...
function applyContactLanguage(form, active) {
    const value = (text) => active ? text : "";
    document.getElementById("form-intent-lead").textContent = value(form.intentLead);
    document.getElementById("choice-intent-A-label").textContent = value(form.intentA);
    document.getElementById("choice-intent-B-label").textContent = value(form.intentB);

    document.getElementById("form-project-label").textContent = value(form.A.project.label);
    document.getElementById("form-project-hint").textContent = value(form.A.project.placeholder);
    document.getElementById("form-starting-point-label").textContent = value(form.A.startingPoint.label);
    document.getElementById("form-starting-point-hint").textContent = value(form.A.startingPoint.placeholder);
    document.getElementById("form-success-A-label").textContent = value(form.A.success.label);
    document.getElementById("form-success-A-hint").textContent = value(form.A.success.placeholder);
    document.getElementById("form-urgency-label").textContent = value(form.A.urgency.label);
    document.getElementById("urgency-asap-label").textContent = value(form.A.urgency.asap);
    document.getElementById("urgency-date-label").textContent = value(form.A.urgency.date);
    document.getElementById("urgency-open-label").textContent = value(form.A.urgency.open);
    document.getElementById("form-target-date-label").textContent = value(form.A.urgency.dateLabel);

    document.getElementById("form-situation-label").textContent = value(form.B.situation.label);
    document.getElementById("form-situation-hint").textContent = value(form.B.situation.placeholder);
    document.getElementById("form-attempts-label").innerHTML = active
        ? `${form.B.attempts.label} <small>(${form.B.attempts.optional})</small>`
        : "";
    document.getElementById("form-attempts-hint").textContent = value(form.B.attempts.placeholder);
    document.getElementById("form-success-B-label").textContent = value(form.B.success.label);
    document.getElementById("form-success-B-hint").textContent = value(form.B.success.placeholder);

    document.getElementById("form-name-label").textContent = value(form.common.nameLabel);
    document.getElementById("form-email-label").textContent = value(form.common.emailLabel);
    elements.name.placeholder = value(form.common.namePlaceholder);
    elements.email.placeholder = value(form.common.emailPlaceholder);
    document.getElementById("form-promise").textContent = value(form.common.promise);
    if (elements.formPrivacy) {
        let privacyText = [...elements.formPrivacy.childNodes].find((node) => node.nodeType === Node.TEXT_NODE);
        if (!privacyText) {
            privacyText = document.createTextNode("");
            elements.formPrivacy.prepend(privacyText);
        }
        privacyText.textContent = active ? `${form.common.privacy} ` : "";
    }
    if (elements.formPrivacyLink) {
        elements.formPrivacyLink.textContent = value(form.common.privacyLink);
        elements.formPrivacyLink.href = routes[currentLang].legal;
    }
    elements.contactForm.querySelector('[type="submit"]').textContent = value(form.common.submit);
    elements.language.value = currentLang;
}

function pruneInactiveViews() {
    inactivePruneTimer = null;
    const activeNarrative = currentPage.key === "A" || currentPage.key === "B";
    if (currentPage.key !== "home") {
        elements.welcomeTitle.textContent = "";
        elements.welcomeProse.replaceChildren();
    }
    if (!activeNarrative) elements.narrative.replaceChildren();
    if (currentPage.key !== "contact") applyContactLanguage(contactForms[currentLang], false);
    if (currentPage.key !== "operations") {
        elements.operationsIntro.textContent = "";
        elements.cabinet.replaceChildren();
        delete elements.cabinet.dataset.operationMode;
    }
}

function scheduleInactiveViewPrune() {
    if (inactivePruneTimer !== null) clearTimeout(inactivePruneTimer);
    if (reducedMotion.matches) {
        pruneInactiveViews();
        return;
    }
    inactivePruneTimer = setTimeout(pruneInactiveViews, 1400);
}

function renderLanguage() {
    const copy = copies[currentLang];
    const form = contactForms[currentLang];
    const isHome = currentPage.key === "home";
    const isNarrative = currentPage.key === "A" || currentPage.key === "B";
    const isContact = currentPage.key === "contact";
    const isOperations = currentPage.key === "operations";
    ...
    elements.contactButton.textContent = isContact ? form.common.submit : copy.contact;
    ...
    elements.brandHome.href = routeUrl(currentLang, "home");
    elements.contactButton.setAttribute(
        "aria-label",
        isContact ? form.common.submit : copy.contactAria
    );
    elements.operationsButton.setAttribute("aria-label", copy.operationsAria);
    elements.operationsButton.href = routeUrl(currentLang, "operations");
    elements.contactButton.href = `${routeUrl(currentLang, "contact")}?intent=${currentIntent}`;
    ...
    if (isContact) {
        applyContactLanguage(form, true);
        setIntent(currentIntent, false);
    }
    if (isNarrative) elements.narrative.innerHTML = copy[currentPage.key];
    if (isOperations) {
...
function shouldHandleClientNavigation(event, target) {
